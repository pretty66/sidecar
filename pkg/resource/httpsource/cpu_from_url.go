...
func (hs *HttpSource) InitData() bool {
	...
	if err != nil {
		logMsg("file cpu init error1", err)
		// cilog.LogErrorw(cilog.LogNameSidecar, "file cpu init error1", err)
		return false
	}

	limitedCoreCount, err = hs.GetLimitedCoreCount()
	if err != nil {
		logMsg("file cpu init error2", err)
		// cilog.LogErrorw(cilog.LogNameSidecar, "file cpu init error2", err)
		return false
	}

	out, err := exec.Command("getconf", "CLK_TCK").Output()
	if err != nil {
		logMsg("file cpu init error3", err)
		// cilog.LogErrorw(cilog.LogNameSidecar, "file cpu init error3", err)
		return false
	}
	cpuTick, err = strconv.Atoi(strings.TrimSpace(string(out)))
	if err != nil {
		logMsg("file cpu init error4", err)
		// cilog.LogErrorw(cilog.LogNameSidecar, "file cpu init error4", err)
		return false
	}
	return limitedCoreCount > 0
}
